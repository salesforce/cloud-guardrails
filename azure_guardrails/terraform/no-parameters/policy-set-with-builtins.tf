locals {
  ...
  policy_names_{{ t.name }} = [{% for service_name, service_policy_names in t.policy_names.items() %}
    # -----------------------------------------------------------------------------------------------------------------
    # {{ service_name }}
...
data "azurerm_policy_definition" "{{ t.name }}" {
  count        = length(local.policy_names_{{ t.name }})
  display_name = element(local.policy_names_{{ t.name }}, count.index)
}
